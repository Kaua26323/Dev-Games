...
async function fetchGameDay() {
  try {
    const response = await fetch(
      `${process.env.NEXT_URL_API}/next-api/?api=game_day`,
      { next: { revalidate: 120 } }
    );
...
async function getGameList() {
  try {
    const res = await fetch(`${process.env.NEXT_URL_API}/next-api/?api=games`, {
      next: { revalidate: 60 },
    });
    ...
    <div className="w-full bg-gray-950 min-h-screen pb-6">
      <Container>
        <h1 className="text-xl font-bold py-5">
          We've picked an exclusive game just for you
        </h1>
